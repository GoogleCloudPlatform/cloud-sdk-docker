...
for i in SCANNING_IMAGES:
  ...
  args: ['build', '--target', 'base', '--build-arg', 'CLOUD_SDK_VERSION=$_CLI_VERSION', {scanning_tags}, '{image_directory}']
  waitFor: ['-']"""
...
for i in IMAGES:
  ...
  id: {image_name}
  args: ['build', '--target', 'base', '--build-arg', 'CLOUD_SDK_VERSION=$_CLI_VERSION', {tags}, '{image_directory}']
  waitFor: ['-']"""
...
for i in MULTI_ARCH:
  ...
  args: ['buildx', 'build', '--target', 'base', '--build-arg', 'CLOUD_SDK_VERSION=$_CLI_VERSION', '--platform', 'linux/arm64,linux/amd64', {tags}, '{image_directory}', '--push']
