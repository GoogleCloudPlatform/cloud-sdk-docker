...
Update the `ARG CLOUD_SDK_VERSION` statement with the version number (e.g. `ARG
CLOUD_SDK_VERSION 160.0.0`) in the following files:
...
To recreate a version-tagged image in Docker Hub, delete the git tag locally and
remotely on GitHub:
...
Then follow the steps in "Releasing new versions" to release the version again.
